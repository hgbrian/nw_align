...
function global_align(seqi, seqj, gap, match, mismatch) {
  ...
  var score = repeat([max_i+1, max_j+1], -1);
  var point = repeat([max_i+1, max_j+1], -1);
  ...
  for (var i=1; i<max_i+1; i++) {
    ...
    for (var j=1; j<max_j+1; j++) {
      ...
      if (dg_score >= up_score && dg_score >= lf_score) {
        score[i][j] = dg_score;
        point[i][j] = DG;
      }
      else if (lf_score >= dg_score && lf_score >= up_score) {
        score[i][j] = lf_score;
        score[i][j] = LF;
      }
      else {
...
function nw_align(seqi, seqj) {
  ...
  console.log(al.align_i+"\n"+al.align_j);
}

// node nw_align.js ACGT TGCA
nw_align(process.argv[2], process.argv[3]);
